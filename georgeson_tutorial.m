%%%%%%%%%%
% SF Mixture tutorial program. PL, 20180618.
% In the style of Georgeson, 1985 (i.e. simulatenous 2-AFC)
% This function will run a simple pilot of the georgeson experiment to
% familiarize the subject with the experimental design/task.
%%%%%%%%%%
...
feedback = 1; % give feedback to the subject?

com_str = 'Tutorial in VNL psychophysics room (1139) in dark conditions. Georgeson style (one interval).';
% comments for meta data file (e.g. room, condition, etc)
%% Pick stimuli
sfNonRef = [1 9 7 2 9 1 5 2]; % indices into the "freqSeries" vector which is created below
                     % as a rule of thumb, freqSeries will have 9 entries
dispBoth = [1 1 1 1 3 3 3 3];
conNonRef = [4 4 3 1 4 3 2 1]; % as indices into TEST_CONS created below (length 4, typically)

NUM_TRIALS = length(sfNonRef);

%% Design of experiment - (blank?:)stim1:blank:stim2
stim_dur = 3; % how many seconds for stimulus?
...
%% For saving results
if is_pilot
    save_base = sprintf('TUTORIAL_s%02d_p%03dg', subj, run_num);
    if save_meta
        sm_base = sprintf('META_TUTORIAL_s%02d_p%03dg', subj, run_num);
    end
else
    save_base = sprintf('TUTORIAL_s%02d_%03dg', subj, run_num);
    if save_meta
        sm_base = sprintf('META_TUTORIAL_s%02d_p%03dg', subj, run_num);
    end
end
...
if save_meta
    ...
    fprintf(meta_inf, 'number of trials: %s\nstimulus eccentricity in degrees: %.3f\n', 'TUTORIAL', stimDist);
...
uniqueDisps = unique(dispBoth);
conVec = [];
for d = 1 : length(uniqueDisps)
    profTemp   = normpdf(octSeries, 0, spreadVec(uniqueDisps(d)));
    profile    = profTemp/sum(profTemp);
    if uniqueDisps(d) == 1 % if it's first family, we make it just 1 grating...
        profile = round(profile);
    end
    conVec{d} = profile .* REF_CON;
end
%% Stencils
...
test_ind = sfNonRef;
test_disp = dispBoth; % both reference and test grating have the same disp, here
test_con = TEST_CONS(conNonRef);
...
tex_ref_single = mglCreateTexture(scaleGrat(grat{ref_ind}, REF_CON));
