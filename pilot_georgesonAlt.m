...
REF_DISP = 4; % 1-4 are our options, as of now (03.12.18)
...
for tr_i = 1:NUM_TRIALS
  ...
  while (etime(clock,stim_on) < stim_dur)
    ...
    if disp1 > 1 % i.e. dispersed grating
        ...
        curr_x = (stim_loc1(1)+stim_radius) + mod(elapsed_time_s*tf/freqCenters(sf1), (slack-1)*2*stim_radius);
...
%     mglStencilSelect(2);
    ...
    if disp2 > 1 % i.e. dispersed grating
        ...
        curr_x = (stim_loc2(1)+stim_radius) + mod(elapsed_time_s*tf/freqCenters(sf2), (slack-1)*2*stim_radius);
